/*
  ...
  This migration creates the complete database schema for the Survey ChatBot application.
  
  ## New Tables
  1. `users` - User profiles and roles
  2. `surveys` - Survey definitions and metadata
  3. `documents` - Uploaded documents and their metadata
  4. `document_chunks` - Text chunks from processed documents
  5. `document_images` - Images extracted from documents
  6. `chat_sessions` - User chat sessions
  7. `chat_messages` - Individual chat messages
  8. `unanswered_queries` - Queries that need admin attention
  9. `user_analytics` - User-specific analytics
  10. `system_analytics` - System-wide analytics
  11. `admin_knowledge` - Admin-provided Q&A knowledge base
...
-- Create RLS policies for anonymous users (for demo purposes)
