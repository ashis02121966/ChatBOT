...
export function ChatProvider({ children }: ChatProviderProps) {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      if (bestResult && bestResult.score > 20) {
        ...
        // Use the helper function to add unanswered query with proper context
        const queryContext = {
          reason: 'no_relevant_context',
          searchAttempted: true,
          chunksFound: allRelevantChunks.length,
          bestScore: rankedResults[0]?.score || 0,
          slmAttempted: true
        };
        
        await addUnansweredQuery(content, currentSession.surveyId, user.id, queryContext);
      }
  ...
  const addUnansweredQuery = async (content: string, surveyId: string, userId: string, additionalContext?: any) => {
    const unansweredQuery: UnansweredQuery = {
      id: crypto.randomUUID(),
      content,
      surveyId: surveyId,
    ...
    const contextData = {
      ...
      surveyName: surveys.find(s => s.id === surveyId)?.name,
      timestamp: new Date().toISOString(),
      userAgent: navigator.userAgent,
      url: window.location.href,
      messageCount: currentSession?.messages.length || 0,
      hasDocuments: searchDocuments('', surveyId, currentSession?.category).length > 0,
      documentCount: searchDocuments('', surveyId, currentSession?.category).length,
      queryLength: content.length,
      queryWords: content.split(/\s+/).length,
      ...additionalContext
    };
    ...
    if (!isCorrect && (message.alternativeAttempts || 0) >= 3) {
      ...
      if (originalQuery && user?.id) {
        ...
        const feedbackContext = {
          ...
          attempts: (message.alternativeAttempts || 0) + 1,
          originalMessageId: messageId,
          feedbackHistory: 'negative_after_max_attempts',
          lastResponse: message.content || message.richContent
        };
        
        await addUnansweredQuery(originalQuery, currentSession.surveyId, user.id, feedbackContext);
      }
    }
  ...
  const generateAlternativeAnswer = async (messageId: string) => {
    ...
    try {
      ...
      if (user?.id) {
        ...
        await addUnansweredQuery(originalQuery, currentSession.surveyId, user.id, errorContext);
      }
    }
  };
